...
function forcePatch(file) {
  let c = fs.readFileSync(file, 'utf8');
  const findA = "Generate the HIGH-LEVEL STRUCTURE for a 60-Day Execution Roadmap.";
  const findB = "          }";
  
  const start = c.indexOf(findA);
  const end = c.indexOf(findB, start);
  
  if (start > -1 && end > -1) {
    const pre = c.substring(0, start);
    const post = c.substring(end + findB.length);
    const newMid = `Generate the HIGH-LEVEL STRUCTURE for a 60-Day Execution Roadmap.
    ...
    fs.writeFileSync(file, pre + newMid + post);
    console.log("Patched", file);
  } else {
    console.log("Failed to find boundaries in", file);
  }
}

forcePatch('api/index.js');
forcePatch('server.js');
